...
export default function ProgressView() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl shadow-lg p-8 border border-green-200 dark:from-slate-900 dark:to-slate-800 dark:border-slate-800">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Learning Progress</h2>
            <p className="text-gray-600 dark:text-gray-300">Track your journey and achievements</p>
          </div>
        </div>
      </div>
      {roadmaps.length > 1 && (
        <div className="mt-4">
          <label className="block text-sm font-semibold text-gray-700 mb-2 dark:text-gray-200">Select Roadmap</label>
          <select
            value={selectedRoadmapId || ''}
            onChange={(e) => setSelectedRoadmapId(parseInt(e.target.value))}
            className="px-4 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white font-medium text-gray-900 dark:bg-slate-900 dark:border-slate-700 dark:text-gray-100"
          >
            {roadmaps.map((r) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all border border-gray-100 p-6 dark:bg-slate-900 dark:border-slate-800">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{stats.totalHours.toFixed(1)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all border border-gray-100 p-6 dark:bg-slate-900 dark:border-slate-800">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{stats.completedTopics}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {weeklyProgress.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 dark:bg-slate-900 dark:border-slate-800">
          <h3 className="text-xl font-bold text-gray-900 mb-6 dark:text-gray-100">
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 dark:bg-slate-900 dark:border-slate-800">
        <h3 className="text-xl font-bold text-gray-900 mb-6 dark:text-gray-100">Topic Progress</h3>
        <div className="space-y-4">
          {progress.length === 0 ? (
            <p className="text-gray-600 text-center py-8 dark:text-gray-300">
            ...
            progress.map((p) => (
              <div key={`${p.roadmap_topic_id}-${p.week_number}`} className="border-b border-gray-200 pb-6 mb-6 last:border-0 last:mb-0 last:pb-0">
                <div className="flex items-center justify-between mb-3">
                  <span className="font-semibold text-gray-900 text-lg dark:text-gray-100">{p.topic_name}</span>
                  <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm font-medium dark:bg-slate-800 dark:text-gray-200">Week {p.week_number}</span>
                </div>
